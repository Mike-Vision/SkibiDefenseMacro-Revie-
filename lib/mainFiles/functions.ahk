...
sd_Pause(*) {
    ...
    if (A_IsPaused) {
        ...
        FwdKeyState := GetKeyState(FwdKey), BackKeyState := GetKeyState(BackKey), LeftKeyState := GetKeyState(LeftKey),
        RightKeyState := GetKeyState(RightKey), SpaceKeyState := GetKeyState(SC_Sapce)
...
sd_DumpRegistry(DumpPath, LogPath) {
    ...
    loop reg DumpPath, "KVR" {
        PrintStr := "Full Path: " A_LoopRegKey "`n`nType: " A_LoopRegType "`n`nRetrieved Item: " A_LoopRegName "`n`nValue: " (((
        A_LoopRegType = "REG_SZ" || A_LoopRegType = "REG_EXPAND_SZ" || A_LoopRegType = "REG_MULTI_SZ" ||
        A_LoopRegType = "REG_DWORD" || A_LoopRegType = "REG_BINARY") ? RegRead(A_LoopRegKey, A_LoopRegName) : "N/A"
        )) "`n`nLast Modified: " A_LoopRegTimeModified "`n`n`n`n"
        try {
...
sd_GrindingInterruptReason(*) {
    ...
    if (FileExist(A_SettingsWorkingDir "interruptreason.txt")) {
        reasonreader := FileOpen(A_SettingsWorkingDir "interruptreason.txt", "r"), InterruptReason := reasonreader.Read(),
        reasonreader.Close()
...
CheckDisplaySpecs() {
    ...
    if offsetfail = 1 {
        MsgBox(
        "Unable to detect in-game GUI offset!`nThis means the macro will NOT work correctly!`n`nThere are a few reasons why this can happen:`n	- Incorrect graphics settings`n	- You are not in the lobby at the moment/Roblox failed to open or the check happened too early (re-run the macro with Roblox pre-opened)`n	- Something is covering the top of your Roblox window`n	- Bitmaps not loaded properly`n`nTo fix:`n	1. Make sure Roblox is fully loaded in the lobby`n	2. Ensure your display scale is 100% (not 125% or 150%)`n	3. Use 16:9 resolution (1920x1080 recommended)`n	4. Close any overlays (Discord overlay, etc.)`n	5. Restart the macro`n`nEnter SD's lobby and try again making sure your display settings are correct.",
        "WARNING!!", 0x1030 " T180")
        ; Do not exit here — allow GUI to finish loading and continue with a safe fallback
        OffsetDetectionFailed := 1
        windowDimensions := "0|0|" A_ScreenWidth "|" A_ScreenHeight
        ; If the GUI is available, show a visible warning and ensure it's enabled so user can interact
        if IsObject(MainGUI) {
            try {
                MainGUI["vCurrentState"].Text := "Warning: Unable to detect in-game GUI offset"
    ...
    windowDimensions := (windowX "|" (IsSet(offsetY) ? (windowY + offsetY) : windowY) "|" windowWidth "|" (IsSet(
    offsetY) ? (windowHeight - offsetY) : windowHeight))
}
...
GenerateRandomString(*) {
    ...
    for k, v in RandomStringArr {
        ...
        if PrevRandomString == RandomString {
            MsgBox("Random string generator generated an identical string to the previous one.", "Error", 0x1000 " T60 Owner" RandomStringGeneratorGUI
                .Hwnd)
            sd_Close()
        }
...
sd_AutoSpeedEventHandler(req) {
    ...
    if req.status = 200 {
        switch Trim(req.responseText, " `t`r`n") {
            ...
            default:
                MsgBox(
                    "Failed to fetch speed events data from GitHub! Your speed will not be automatically updated accordingly to any new global speed events.`n`NIf you would like to use a speed event, update your speed manually.",
                    "Speed Events", 0x1010 " T60")
        }
    }
}
...
sd_DefaultHandlers() {
    ...
    if (DiscordCheck = 0) && (MainChannelCheck = 1) || (DiscordCheck = 1) && (MainChannelCheck = 0) || (DiscordCheck =
        0) && (MainChannelCheck = 0) {
        MainChannelEditDisabled := "Disabled"
    } else {
        MainChannelEditDisabled := ""
    }
    if (DiscordCheck = 0) && (ReportChannelCheck = 1) || (DiscordCheck = 1) && (ReportChannelCheck = 0) || (
        DiscordCheck = 0) && (ReportChannelCheck = 0) {
        ReportsChannelEditDisabled := "Disabled"
    } else {
        ReportsChannelEditDisabled := ""
    }
    if (DiscordCheck = 0) && (Criticals = 1) || (DiscordCheck = 1) && (Criticals = 0) || (DiscordCheck = 0) && (
        Criticals = 0) {
        PingsDisabled := "Disabled"
    } else {
        PingsDisabled := ""
    }
    if (DiscordCheck = 0) && (Screenshots = 1) || (DiscordCheck = 1) && (Screenshots = 0) || (DiscordCheck = 0) && (
        Screenshots = 0) {
...
sd_Quickstart() {
    ...
    QuickStart2(*) {
        ...
        QuickstartGUI2.AddText("x10 y10",
            "First, choose the chapters you want to grind and how you want to grind them:")
        (ChapterName1Edit := QuickstartGUI2.AddDropDownList("x18 y50 w106 vChapterName1", ChapterNamesList)).Text :=
            ChapterName1, ChapterName1Edit.OnEvent("Change", sd_ChapterSelect_1)
        (ChapterName2Edit := QuickstartGUI2.AddDropDownList("xp yp+25 wp vChapterName2 Disabled", ["None"])).Add(
            ChapterNamesList), ChapterName2Edit.Text := ChapterName2, ChapterName2Edit.OnEvent("Change",
                sd_ChapterSelect_2)
        (ChapterName3Edit := QuickstartGUI2.AddDropDownList("xp yp+25 wp vChapterName3 Disabled", ["None"])).Add(
            ChapterNamesList), ChapterName3Edit.Text := ChapterName3, ChapterName3Edit.OnEvent("Change",
                sd_ChapterSelect_3)
        (GrindModeEdit := QuickstartGUI2.AddDropDownList("x200 y50 vGrindMode Disabled", GrindModesArr)).Text :=
            GrindMode, GrindModeEdit.OnEvent("Change", sd_GrindMode)
        ...
        QuickStart3(*) {
            global
            local UnitModeEdit, UnitSlot1Edit, UnitSlot2Edit, UnitSlot3Edit, UnitSlot4Edit, UnitSlot5Edit,
                UnitSlot6Edit, UnitSlot7Edit, UnitSlot8Edit, UnitSlot9Edit, UnitSlot0Edit
            QuickstartGUI2.Destroy(), QuickstartGUI2 := ""
            GUIClose3(*) {
            ...
            (UnitModeEdit := QuickstartGUI3.AddDropDownList("x150 y196 vUnitModeEdit Disabled", UnitModesArr)).Text :=
                UnitMode, UnitModeEdit.OnEvent("Change", sd_UnitMode)
            ...
            (UnitSlot1Edit := QuickstartGUI3.AddDropDownList("x21 y46 vUnitSlot1Edit Disabled", UnitNamesList)).Text :=
                UnitSlot1, UnitSlot1Edit.OnEvent("Change", sd_UnitSlotSelect_1)
            (UnitSlot2Edit := QuickstartGUI3.AddDropDownList("x21 y76 vUnitSlot2Edit Disabled", ["None"])).Add(
                UnitNamesList), UnitSlot2Edit.Text := UnitSlot2, UnitSlot2Edit.OnEvent("Change", sd_UnitSlotSelect_2)
            (UnitSlot3Edit := QuickstartGUI3.AddDropDownList("x165 y76 vUnitSlot3Edit Disabled", ["None"])).Add(
                UnitNamesList), UnitSlot3Edit.Text := UnitSlot3, UnitSlot3Edit.OnEvent("Change", sd_UnitSlotSelect_3)
            (UnitSlot4Edit := QuickstartGUI3.AddDropDownList("x21 y106 vUnitSlot4Edit Disabled", ["None"])).Add(
                UnitNamesList), UnitSlot4Edit.Text := UnitSlot4, UnitSlot4Edit.OnEvent("Change", sd_UnitSlotSelect_4)
            (UnitSlot5Edit := QuickstartGUI3.AddDropDownList("x165 y106 vUnitSlot5Edit Disabled", ["None"])).Add(
                UnitNamesList), UnitSlot5Edit.Text := UnitSlot5, UnitSlot5Edit.OnEvent("Change", sd_UnitSlotSelect_5)
            (UnitSlot6Edit := QuickstartGUI3.AddDropDownList("x21 y136 vUnitSlot6Edit Disabled", ["None"])).Add(
                UnitNamesList), UnitSlot6Edit.Text := UnitSlot6, UnitSlot6Edit.OnEvent("Change", sd_UnitSlotSelect_6)
            (UnitSlot7Edit := QuickstartGUI3.AddDropDownList("x165 y136 vUnitSlot7Edit Disabled", ["None"])).Add(
                UnitNamesList), UnitSlot7Edit.Text := UnitSlot7, UnitSlot7Edit.OnEvent("Change", sd_UnitSlotSelect_7)
            (UnitSlot8Edit := QuickstartGUI3.AddDropDownList("x21 y166 vUnitSlot8Edit Disabled", ["None"])).Add(
                UnitNamesList), UnitSlot8Edit.Text := UnitSlot8, UnitSlot8Edit.OnEvent("Change", sd_UnitSlotSelect_8)
            (UnitSlot9Edit := QuickstartGUI3.AddDropDownList("x165 y166 vUnitSlot9Edit Disabled", ["None"])).Add(
                UnitNamesList), UnitSlot9Edit.Text := UnitSlot9, UnitSlot9Edit.OnEvent("Change", sd_UnitSlotSelect_9)
            (UnitSlot0Edit := QuickstartGUI3.AddDropDownList("x28 y196 vUnitSlot0Edit Disabled", ["None"])).Add(
                UnitNamesList), UnitSlot0Edit.Text := UnitSlot0, UnitSlot0Edit.OnEvent("Change", sd_UnitSlotSelect_0)
        ...
        CreateFolder(folder*) {
            if (!FileExist(folder)) {
                try {
                    ...
                    MsgBox("Could not create the " folder " directory!`nThis means the macro will not be able to use the functions of the files usually in this folder!`nTry moving the Macro to a different folder (e.g. Downloads or Documents).",
                        "Failed to Create Folder", 0x1012)
                }
            }
        }
                ...
                switch (c := SubStr(A_LoopField, 1, 1)) {
                    ...
                    default:
                        if (p := InStr(A_LoopField, "=")) {
                            try {
                                k := SubStr(A_LoopField, 1, p - 1), %k% := IsInteger(v := SubStr(A_LoopField, p + 1)) ? Integer(
                                    v) : v
                            }
                        }
                }
        ...
        sd_ImgSearch(imageName, v, aim := "full", trans := "none") {
            ...
            xi := (aim = "highright") ? windowWidth // 2 : (aim = "right") ? windowWidth // 2 : (aim = "center") ? windowWidth //
                4 : (aim = "lowright") ? windowWidth // 2 : 0
            yi := (aim = "low") ? windowHeight // 2 : (aim = "center") ? windowHeight // 4 : (aim = "lowright") ? windowHeight //
                2 : 0
            ww := (aim = "highleft") ? windowWidth // 2 : (aim = "left") ? windowWidth // 2 : (aim = "center") ? xi * 3 :
                windowWidth
            wh := (aim = "high") ? windowHeight // 2 : (aim = "highright") ? windowHeight // 2 : (aim = "highleft") ?
                windowHeight // 2 : (aim = "center") ? yi * 3 : windowHeight
            if DirExist(A_MacroWorkingDir "sd_img_assets") {
                try {
                    result := ImageSearch(&FoundX, &FoundY, windowX + xi, windowY + yi, windowX + ww, windowY + wh, "*" v ((
                        trans != "none") ? (" *Trans" trans) : "") " " A_MacroWorkingDir "sd_img_assets\" imageName)
                } catch {
                    Sleep 5000
        ...
        DisconnectCheck(testCheck := 0) {
            ...
            static ServerLabels := Map(0, "Public Server", 1, "Private Server", 2, "Fallback Server 1", 3, "Fallback Server 2",
                4, "Fallback Server 3")
            ...
            loop {
                ; Decide Server
                server := ((A_Index <= 20) && linkCodes.Has(n := (A_Index - 1) // 5 + 1)) ? n : ((PublicFallback = 0) && (n :=
                    ObjMinIndex(linkcodes))) ? n : 0
                ...
                loop 5 {
                            ...
                            if (server) {
                                ...
                                if ((success := BrowserReconnect(linkCodes[server], i)) = 1) {
                                    if ReconnectMethod != "Browser" {
                                        ...
                                        sd_SetStatus("Warning",
                                            "Deeplink reconnect failed, switched to browser reconnect for this session!")
                                    }
                                    break
                                } else {
                                    continue 2
                    ...
                    loop 180 {
                        ...
                        pBMScreen := Gdip_BitmapFromScreen((IsSet(windowDimensions) ? windowDimensions : windowX "|" (IsSet(
                            offsetY) ? (windowY + offsetY) : windowY) "|" windowWidth "|" (IsSet(offsetY) ? (windowHeight -
                    ...
                    loop 180 {
                        ...
                        pBMScreen := Gdip_BitmapFromScreen((IsSet(windowDimensions) ? windowDimensions : windowX "|" (IsSet(
                            offsetY) ? (windowY + offsetY) : windowY) "|" windowWidth "|" (IsSet(offsetY) ? (windowHeight -
                                offsetY) : windowHeight)))
                        if (Gdip_ImageSearch(pBMScreen, bitmaps["LoadingData"], , , , , , 4) = 0) || (Gdip_ImageSearch(
                            pBMScreen, bitmaps["ChapterButton"], , , , , , 6) = 1) {
                            Gdip_DisposeImage(pBMScreen)
                            sd_SetStatus("Detected", "Game Loaded")
                            success := 1
                            break 2
                ...
                if success = 1 {
                    ...
                    MouseMove((windowX + (windowWidth // 2)), (IsSet(offsetY) ? ((windowY + offsetY) + ((windowHeight - offsetY
                    ) // 2)) : (windowY + (windowHeight // 2))))
        ...
        sd_ContributorsHandler(req) {
            ...
            sd_ContributorsImage(1, (req.status = 200) ? StrSplit(req.responseText, "`n", " `t")
                : ["Error while loading,red", "contributors!,red", "", "Make sure you have,red", "a working internet,red",
                    "connection and then,red", "reload the macro.,red"])
        }
        ...
        sd_ContributorsImage(page := 1, contributors := "") {
            ...
            if (!IsSet(hBM1)) {
                ...
                testers := [["kapcho", 0xffff00ff, "1146158446522138714"], ["kapcho", 0xffa45ee9, "1146158446522138714"], [
                    "kapcho", 0xffdfdfdf, "1146158446522138714"], ["kapcho", 0xff3f8d4d, "1146158446522138714"], ["kapcho",
                        0xff7aa22c, "1146158446522138714"], ["kapcho", 0xff2bc016, "1146158446522138714"], ["kapcho",
                            0xffffdc64, "1146158446522138714"], ["ulacon", 0xff794044, "822378669091061760"], ["ulacon",
                                0xffffde48, "822378669091061760"], ["ulacon", 0xff0096ff, "822378669091061760"], ["ulacon",
                                    0xfff47fff, "822378669091061760"], ["ulacon", 0xffa3bded, "822378669091061760"], ["ulacon",
                                        0xfff49fbc, "822378669091061760"]]
                ...
                pBrush := Gdip_CreateLinearGrBrushFromRect(x + 1, y, SubStr(pos, InStr(pos, "|", , , 2) + 1, InStr(pos, "|", , ,
                    3) - InStr(pos, "|", , , 2) - 1) + 2, 14, 0x00000000, 0x00000000, 2)
                Gdip_SetLinearGrBrushPresetBlend(pBrush, [0.0, 0.5, 1], [0xfff0ca8f, 0xffd48d22, 0xfff0ca8f])
                Gdip_FillRoundedRectangle(G, pBrush, x + 1, y, SubStr(pos, InStr(pos, "|", , , 2) + 1, InStr(pos, "|", , , 3) -
                InStr(pos, "|", , , 2) - 1), 14, 4)
                Gdip_DeleteBrush(pBrush)
                ...
                for v in devs {
                    ...
                    if (x + (w := Number(SubStr(pos, InStr(pos, "|", , , 2) + 1, InStr(pos, "|", , , 3) - InStr(pos, "|", , , 2
                    ) - 1))) > 239) {
                        x := 5, y += 13
                    }
                    pBrush := Gdip_CreateLinearGrBrushFromRect(0, y + 1, 242, 12, 0xff000000 + (Min(Round(Gdip_RFromARGB(v[2]) *
                    1.2), 255) << 16) + (Min(Round(Gdip_GFromARGB(v[2]) * 1.2), 255) << 8) + Min(Round(Gdip_BFromARGB(v[2]) * 1,
                    2), 255)
                    , 0xff000000 + (Min(Round(Gdip_RFromARGB(v[2]) * 0.9), 255) << 16) + (Min(Round(Gdip_GFromARGB(v[2]) * 0.9),
                    255) << 8) + Min(Round(Gdip_BFromARGB(v[2]) * 0.9), 255)), pPen := Gdip_CreatePenFromBrush(pBrush, 1)
                ...
                pBrush := Gdip_CreateLinearGrBrushFromRect(x + 1, y, SubStr(pos, InStr(pos, "|", , , 2) + 1, InStr(pos, "|", , ,
                    3) - InStr(pos, "|", , , 2) - 1), 14, 0x00000000, 0x00000000, 2)
                Gdip_SetLinearGrBrushPresetBlend(pBrush, [0.0, 0.5, 1], [0xfff0ca8f, 0xffd48d22, 0xfff0ca8f])
                Gdip_FillRoundedRectangle(G, pBrush, x + 1, y, SubStr(pos, InStr(pos, "|", , , 2) + 1, InStr(pos, "|", , , 3) -
                InStr(pos, "|", , , 2) - 1) + 1, 14, 4)
                Gdip_DeleteBrush(pBrush)
                ...
                for v in testers {
                    ...
                    pBrush := Gdip_CreateLinearGrBrushFromRect(0, y + 1, 242, 12, 0xff000000 + (Min(Round(Gdip_RFromARGB(v[2]) *
                    1.2), 255) << 16) + (Min(Round(Gdip_GFromARGB(v[2]) * 1.2), 255) << 8) + Min(Round(Gdip_BFromARGB(v[2]) *
                    1.2), 255)
                    , 0xff000000 + (Min(Round(Gdip_RFromARGB(v[2]) * 0.9), 255) << 16) + (Min(Round(Gdip_GFromARGB(v[2]) * 0.9),
                    255) << 8) + Min(Round(Gdip_BFromARGB(v[2]) * 0.9), 255)), pPen := Gdip_CreatePenFromBrush(pBrush, 1)
                    Gdip_DrawOrientedString(G, v[1], "Tahoma", 11, 0, x, y, 130, 10, 0, pBrush, pPen)
                ...
                for k, v in contributors {
                    ...
                    pBrush := Gdip_CreateLinearGrBrushFromRect(x, y + 1, SubStr(pos, InStr(pos, "|", , , 2) + 1, InStr(pos, "|", , ,
                        3) - InStr(pos, "|", , , 2) - 1), 12, 0x00000000, 0x00000000, 2)
                ...
                DllCall("DeleteDC", "UInt", hdcSrc), DllCall("ReleaseDC", "UInt", MainGUI["ContributorsImage"].Hwnd, "UInt",
                    hdcDst)
                DllCall("SendMessage", "UInt", MainGUI["ContributorsImage"].Hwnd, "UInt", 0x0B, "UInt", 0, "UInt", 0)        ; WM_SETREDRAW OFF
                oBM := DllCall("SendMessage", "UInt", MainGUI["ContributorsImage"].Hwnd, "UInt", 0x172, "UInt", 0, "UInt", hBM%
                    page%) ; STM_SETIMAGE
        ...
        BrowserReconnect(linkCode, i) {
            global bitmaps
            static cmd := Buffer(512), init := (DllCall("shlwapi\AssocQueryString", "Int", 0, "Int", 1, "Str", "http", "Str",
                "open", "Ptr", cmd.Ptr, "IntP", 512),
            DllCall("Shell32\SHEvaluateSystemCommandTemplate", "Ptr", cmd.Ptr, "PtrP", &pEXE := 0, "Ptr", 0, "PtrP", &pPARAMS :=
                0))
            ...
            loop 1 {
                ...
                loop 240 {
                    ...
                    pBMScreen := Gdip_BitmapFromScreen((IsSet(windowDimensions) ? windowDimensions : windowX "|" (IsSet(offsetY
                        ...
                        windowHeight)))
                    if (Gdip_ImageSearch(pBMScreen, bitmaps["LoadingData"], , , , , , 4) = 0 || Gdip_ImageSearch(pBMScreen,
                        bitmaps["ChapterButton"], , , , , , 2) = 1) {
                        Gdip_DisposeImage(pBMScreen)
                        sd_SetStatus("Detected", "Game Loaded")
                        success := 1
                        break 2
        ...
        sd_RunDiscord(linkPath) {
            static cmd := Buffer(512), init := (DllCall("shlwapi\AssocQueryString", "Int", 0, "Int", 1, "Str", "discord", "Str",
                "open", "Ptr", cmd.Ptr, "IntP", 512),
            DllCall("Shell32\SHEvaluateSystemCommandTemplate", "Ptr", cmd.Ptr, "PtrP", &pEXE := 0, "Ptr", 0, "PtrP", &pPARAMS :=
                0))
            , exe := (pEXE > 0) ? StrGet(pEXE) : ""
        ...
        ElevateScript() {
            try {
                ...
                if !(A_IsAdmin) {
                    MsgBox(
                        "You must run Skibi Defense Macro as administrator in this folder!`nIf you don't want to do this, move the macro to a different folder (e.g. Downloads, Desktop)",
                        "Error", 0x40010)
                    sd_Close()
                }
                ; elevated but still can't write, read-only directory?
                MsgBox(
                    "You cannot run Skibi Defense Macro in this folder!`nTry moving the macro to a different folder (e.g. Downloads, Desktop)",
                    "Error", 0x40010)
            }
            else {
                file.Close()
        ...
        sd_ImportStrategies() {
            ...
            loop files A_MacroWorkingDir "strats\*.ahk" {
                ...
                if (!InStr(imported, imported_strat := '("' (strat_name := StrReplace(A_LoopFileName, "." A_LoopFileExt)) '")`r`n' strat '`r`n`r`n'
                    ...
                    exec := ComObject("WScript.Shell").Exec('"' exe_path64 '" /script /Validate /ErrorStdOut *'), exec.StdIn.Write(
                        script), exec.StdIn.Close()
        ...
        sd_ImportPaths() {
            static path_names := Map("jc", ["Char1", "Char2", "Char3", "Char4", "Char5", "Char6", "NM3", "NM4", "NM1", "NM2",
                "NM6", "NM5", "Endless", "EndShield"],  ; join chapter
            "rtf", ["Char1", "Char2", "Char3", "Char4", "Char5", "Char6", "NM3", "NM4", "NM1", "NM2", "NM6", "NM5", "Endless",
                "EndShield"]) ; return from chapter
                ...
                for v in list {
                    try {
                        ...
                        if regexMatch(paths[k][v], "im)paths\[")
                            MsgBox('Path "' k '-' v '" appears to be deprecated!`nThis means the macro will NOT work correctly!`nCheck for an updated version of the path or`nrestore the default path',
                                "Error", 0x40010 " T60")
                    } catch {
                        MsgBox('Could not find the "' k '-' v '" path!`nThis means the macro will NOT work correctly!`nMake sure the path exists in the "paths" folder and redownload if it does nott!',
                            "Error", 0x40010 " T60")
                    }
                }
            }
        ...
        sd_WM_COPYDATA(wParam, lParam, *) {
            ...
            DetectHiddenWindows(0)
            InStr(StringText, ": ") ? sd_SetStatus(SubStr(StringText, 1, InStr(StringText, ": ") - 1), SubStr(StringText, InStr(
                StringText, ": ") + 2)) : sd_SetStatus(StringText)
            return 0
        }
